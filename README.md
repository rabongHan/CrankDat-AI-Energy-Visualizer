...
The system combines:
1) **Energy-Estimation Chrome Extension** *(prototype complete, improvement required)*  
   - Estimates energy usage (Wh) per ChatGPT prompt  
...
## 🏗️ Project Status
- ✅ Chrome Extension prototype complete  
- 🔨 Crank hardware prototype in progress  
- 📠 Code Section in README should be revised as we update codes; currently, generated by ChatGPT
---
...
| `content.js`        | Detects ChatGPT assistant replies, estimates tokens, computes Wh, logs to `chrome.storage.local`, and updates the in-page overlay with **This Chat Total**.                          |
| `popup.html`        | Popup UI when clicking the extension icon.                                                                                                                                            |
| `popup.js`          | Renders recent entries and **per-chat total**; color-codes rows by conversation.                                                                                                      |
...
| `overlay.css`       | Styles the floating overlay shown on ChatGPT (bottom-right).                                                                                                                          |
...
* Watches for new assistant messages (MutationObserver + fallback).
...
* Stores entries locally:
  ...
  ```
* Updates overlay text: `Prompt Energy (This Chat Total): X.XXX Wh`.

### `popup.js`

* Reads `history` from `chrome.storage.local`.
* Computes **This Chat Total** (active tab’s conversation).
* Shows recent entries (Wh + token count), color-coded per conversation.
